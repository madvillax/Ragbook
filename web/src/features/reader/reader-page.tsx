  ...
  return (
    <main className="grid min-h-[100dvh] place-items-center bg-[#e9ebef] p-6 dark:bg-black">
      <div className="max-w-md rounded-2xl border border-[var(--line)] bg-[var(--surface-raised)] p-8 text-center shadow-[var(--shadow-panel)]">
        ...
        <Link href="/" className="mt-6 inline-flex items-center gap-2 text-sm font-medium text-accent-600 dark:text-accent-100"><ArrowLeft size={16} /> Back to library</Link>
      </div>
    </main>
  );
}
...
export function ReaderPage({ bookId }: { bookId: string }) {
  ...
  const documentQuery = useDocument(bookId);
  ...
  if (documentQuery.error) return <ReaderMessage title="Document unavailable" detail={documentQuery.error.message} warning />;

  const currentDocument = documentQuery.data;
  if (!currentDocument) return <ReaderMessage title="Document unavailable" detail="The document could not be loaded." warning />;
